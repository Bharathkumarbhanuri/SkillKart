const { enrollCourse,getCourseTitleandPrice, checkEnrollment, fetchEnrolledCourses } = require('../models/enrollmentModel');
const Cart = require('../models/cartModel');
const axios = require('axios');
const db = require('../config/db');

const sendEventToWorkflowService = async (req, user_id, course_id, course_title, total_amount) => {
    try {
        ...
        const response = await axios.post('http://localhost:8080/api/events', eventPayload, {
            headers: { 'X-API-Key': process.env.WORKFLOW_API_KEY }
        });
...
const enroll = async (req, res) => {
    try {
        ...
        for (let course_id of courses) {
            ...
            const courseDetails = await getCourseTitleandPrice(course_id);

            const course_title = courseDetails.title;
            const total_amount = courseDetails.price;

            await Cart.removeFromCart(user_id, course_id);

            // Send event to the workflow service
            await sendEventToWorkflowService(req, user_id, course_id, course_title, total_amount);
        }
